...
Although these transforms are mostly "robotic" - which to implement is a matter of judgement. Choosing good names, or the optimal function boundaries for useful functional abstraction,  requires a high level skills and a detailed understanding of the program semantics.
...
Writing good code is about more than these transformations. For example, the correct types must be identified and written. Monadic types, where useful, must be used. These transformations nevertheless are crucial to keep in mind when writing the actual code.
